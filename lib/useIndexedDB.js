...
function getDB() {
  return openDB(DB_NAME, DB_VERSION, {
    upgrade(db) {
      ...
      if (!db.objectStoreNames.contains('recent_translations')) {
        const store = db.createObjectStore('recent_translations', { keyPath: 'id', autoIncrement: true });
        store.createIndex('timestamp', 'timestamp');
      }
    },
  });
...
export function useIndexedDB() {
  ...
  // ── Recent Translations ──
  const addRecentTranslation = useCallback(async (translation) => {
    const db = await getDB();
    const record = { ...translation, timestamp: new Date().toISOString() };
    const id = await db.add('recent_translations', record);
    return { ...record, id };
  }, []);

  const getRecentTranslations = useCallback(async () => {
    const db = await getDB();
    const translations = await db.getAll('recent_translations');
    return translations.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  }, []);

  const deleteRecentTranslation = useCallback(async (id) => {
    const db = await getDB();
    return db.delete('recent_translations', id);
  }, []);
  ...
  return {
    ...
    addRecentTranslation, getRecentTranslations, deleteRecentTranslation,
  };
}
